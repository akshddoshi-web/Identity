...
try:
    ...
    st.error(
        f"Could not read the database ({e}). Run `python scripts/generate_sample_data.py` "
        "and `python scripts/run_pipeline_demo.py` first, or point config/config.yaml at a "
        "real database with live ingestion already run."
    )
    st.stop()
...
st.caption(
    ...
    "nothing is retroactively added. On synthetic demo data these are historical games, not "
    "real upcoming games; wire in live odds ingestion (data_pipeline/ingest_odds.py) for an "
    "actual pre-game slate."
)
...
if ledger_view.empty:
    ...
    tabs = st.tabs(sport_filter if sport_filter else ["All"])
    for tab, sport in zip(tabs, sport_filter if sport_filter else ["All"]):
        with tab:
